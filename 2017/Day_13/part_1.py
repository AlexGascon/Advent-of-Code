...
range. (Ignore layers in which you do not get caught.) The severity of the
...
def test_compute_severity():
    ...
    assert compute_severity(example) == 24


def test_parse_firewall_structure():
    example_structure = """0: 3
    1: 2
    4: 4
    6: 4"""
    ...
    assert parse_firewall_structure(example_structure) == expected_structure


def test_round_trip_time():
    assert round_trip_time(3) == 4
    assert round_trip_time(2) == 2
    assert round_trip_time(4) == 6
...
def compute_severity(firewall_structure):
    firewall = parse_firewall_structure(firewall_structure)

    severity = 0

    for layer, length in firewall.items():
        if layer % round_trip_time(length) == 0:
            severity += length*layer
...
if __name__ == '__main__':
    ...
    severity = compute_severity(input_content)
