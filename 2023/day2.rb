...
class Game
  ...
  def power
    min_balls_per_color
      .values
      .reduce(1) { |acc, color| acc * color }
  end

  private

  def min_balls_per_color
    sets
      .reduce({ blue: 0, green: 0, red: 0 }) do |maxes, set|
        {
          blue: [maxes[:blue], set.blue].max,
          green: [maxes[:green], set.green].max,
          red: [maxes[:red], set.red].max
        }
      end
  end
end
...
class CubeSet
  ...
  def valid?
    green <= MAX_CUBES[:green] && blue <= MAX_CUBES[:blue] && red <= MAX_CUBES[:red]
  end
end
...
class Day2
  def self.part1
    File
      ...
      .filter { |game| game.sets.all?(&:valid?) }
